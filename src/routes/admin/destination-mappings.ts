...
import type { DestinationMappingRepository, DestinationMappingRecord } from '../../repositories/destination-mapping.repository.js'
import { adminDestinationMappingCreateSchema, adminDestinationMappingListQuerySchema, adminDestinationMappingUpdateSchema } from '../../schemas/admin.js'
import { parseJson, parseQuery } from './helpers.js'

export function mountAdminDestinationMappingRoutes(app: Hono, mappings: DestinationMappingRepository): void {
  app.get('/admin/merchants/:merchantId/destination-mappings', async (c) => {
    ...
    const result = await mappings.list({
  ...
  app.patch('/admin/destination-mappings/:mappingId', async (c) => {
    ...
    const mapping = await mappings.update(c.req.param('mappingId'), {
      country: input.country,
      province: input.province,
      city: input.city,
      district: input.district,
