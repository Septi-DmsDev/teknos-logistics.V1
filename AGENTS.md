...
- 2026-06-23: Sprint 11B SAP Express sandbox validation progressed past offline-only status. SAPX sandbox rates for Teknos origin `JI1606` now pass for requested destinations with NON-COD `DEV000` and COD `DEV001`; sandbox bookings created NON-COD AWB `DEV00845560419` and COD AWB `DEV00845560420`; tracking for both AWBs now returns 7 events ending at `POD - DELIVERED`. Code supports split customer-code envs, COD `cod_value`, and webhook `created_at` timestamp alias. Remaining SAPX item: inbound webhook/push tracking setup to `/webhooks/sap-express` on `https://logistic-api.teknos.id`; do not run more SAPX create/request order calls without explicit operator approval. Detailed handoff: `docs/handover/2026-06-22-sapx-sandbox-webhook-progress.md`.
