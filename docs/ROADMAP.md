...
| Sprint 6 | `teknos.id` staging integration | In progress - `GET /openapi.json` and `npm run contract:check` added for parent handoff; parent `teknos.id` remains read-only until separately approved | In Progress |
