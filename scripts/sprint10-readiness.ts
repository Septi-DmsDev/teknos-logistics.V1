...
const checks = [
  ...
  checkFileIncludes('src/couriers/sap-express/sap-express.adapter.ts', ['SapExpressAdapter', 'getRates', 'bookShipment', 'normalizeWebhook']),
  checkFileIncludes('src/app.ts', ['new JntAdapter()', 'new SapExpressAdapter(env)', 'mountCourierRoutes(app)']),
  ...
  checkFileIncludes('prisma/schema.prisma', ['model DestinationMapping', 'destinationMappings', '@@unique([merchantId, courier, sourceKey])']),
  checkFileIncludes('prisma/migrations/20260620103000_add_destination_mappings/migration.sql', ['CREATE TABLE "DestinationMapping"']),
  checkFileIncludes('prisma/migrations/20260620152000_make_destination_mappings_importable/migration.sql', ['DestinationMapping_merchantId_courier_providerCode_idx', 'DestinationMapping_merchantId_courier_sourceKey_key']),
